    ...
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 border-b ${isScrolled ? 'bg-[#fcfaf8]/90 backdrop-blur-md border-[#A68966]/20' : 'bg-transparent backdrop-blur-sm border-white/10'}`}>
      ...
      >
        <nav className="flex flex-col gap-4 mt-2">
            ...
            {user?.publicMetadata?.role === 'admin' && (
              <>
                <Link onClick={() => setIsMobileMenuOpen(false)} href="/admin/reservations" className="text-[#e6a219] text-xl font-serif italic border-b border-[#E8E1D9] pb-3 flex items-center justify-between">
                  Panel de Reservas (Admin)
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5"><rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" /><rect x="14" y="14" width="7" height="7" /><rect x="3" y="14" width="7" height="7" /></svg>
                </Link>
                <Link onClick={() => setIsMobileMenuOpen(false)} href="/admin/vehicles" className="text-[#e6a219] text-xl font-serif italic border-b border-[#E8E1D9] pb-3 flex items-center justify-between">
                  Gestión de Flota (Admin)
